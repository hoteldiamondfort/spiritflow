...
export default function StockInsightPage() {
  ...
  const generatePDF = async () => {
    try {
      ...
      // ===== METADATA BOX (Light gray background with proper vertical centering) =====
      const metadataBoxHeight = 12;
      ...
      // Calculate vertical center with exact centering inside box
      const metadataCenterY = metadataBoxY + metadataBoxHeight / 2;

      // LEFT SIDE: Report Date & Report Time (exactly centered vertically)
      doc.text(`REPORT DATE: ${reportDate}`, margin + 2, metadataCenterY - 2);
      doc.text(`REPORT TIME: ${reportTime}`, margin + 2, metadataCenterY + 2);
      
      // RIGHT SIDE: Stock Category & Generated by (exactly centered vertically, right-aligned)
      doc.text(`STOCK CATEGORY: ${categoryDisplay}`, pageWidth - margin - 2, metadataCenterY - 2, { align: 'right' });
      doc.text(`GENERATED BY: ${userNameDisplay}`, pageWidth - margin - 2, metadataCenterY + 2, { align: 'right' });
