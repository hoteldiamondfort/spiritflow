...
export default function StockInsightPage() {
  ...
  const generatePDF = async () => {
    try {
      ...
      // ===== METADATA BOX (Light gray background with vertical centering) =====
      ...
      // Calculate vertical center for equal top/bottom padding
      const metadataCenterY = metadataBoxY + metadataBoxHeight / 2;

      // LEFT SIDE: Report Date & Report Time (vertically centered)
      doc.text(`REPORT DATE: ${reportDate}`, margin + 2, metadataCenterY - 2.5);
      doc.text(`REPORT TIME: ${reportTime}`, margin + 2, metadataCenterY + 1.5);
      
      // RIGHT SIDE: Stock Category & Generated by (vertically centered, right-aligned)
      doc.text(`STOCK CATEGORY: ${categoryDisplay}`, pageWidth - margin - 2, metadataCenterY - 2.5, { align: 'right' });
      doc.text(`GENERATED BY: ${userNameDisplay}`, pageWidth - margin - 2, metadataCenterY + 1.5, { align: 'right' });
