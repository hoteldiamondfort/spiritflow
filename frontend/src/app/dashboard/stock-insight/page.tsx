...
export default function StockInsightPage() {
  ...
  const generatePDF = async () => {
    try {
      ...
      // ===== METADATA BOX (Professional layout with separate box and spacing) =====
      const metadataBoxHeight = 11;
      ...
      doc.rect(margin, metadataBoxY, pageWidth - margin * 2, metadataBoxHeight);
      
      // Metadata text with padding
      ...
      // LEFT SIDE: Report Date & Report Time (with padding inside box)
      doc.text(`REPORT DATE: ${reportDate}`, margin + 2, metadataBoxY + 3.5);
      doc.text(`REPORT TIME: ${reportTime}`, margin + 2, metadataBoxY + 7);
      
      // RIGHT SIDE: Stock Category & Generated by (right-aligned with padding)
      doc.text(`STOCK CATEGORY: ${categoryDisplay}`, pageWidth - margin - 2, metadataBoxY + 3.5, { align: 'right' });
      doc.text(`GENERATED BY: ${userNameDisplay}`, pageWidth - margin - 2, metadataBoxY + 7, { align: 'right' });
      ...
      // ===== TABLE CONFIG (Product: 40%, Stock columns: 15% each = 60%) =====
